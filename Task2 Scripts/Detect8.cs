...
public class Detect8 : MonoBehaviour
{
	...
	private void Start() {
		...
		tr = GameObject.Find("8").transform;
		one   = false;
		two   = false;
		three = false;
		four  = false;
	...
	void resetBooks() {
		...
		//move others
		tr.position = tr.position + new Vector3(11.28f,0,0);
		StartCoroutine("WaitForAnotherSec");
	}

	//Logic for book detection of the 8th sequence
	...
	{	
		...
		if(Other.CompareTag("2") && two == false)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logWrongTime();
			wrongNotify2.SetActive(true);
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logChange();
		...
		if(Other.CompareTag("2") && two == true)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logCorrectTime();
			correctNotify2.SetActive(true);
			three = true;
		...
		if(Other.CompareTag("D") && three == false)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logWrongTime();
			wrongNotify2.SetActive(true);
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logChange();
		...
		if(Other.CompareTag("D") && three == true)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logCorrectTime();
			correctNotify2.SetActive(true);
			four = true;
		...
		if(Other.CompareTag("B") && four == false)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logWrongTime();
			wrongNotify2.SetActive(true);
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logChange();
		...
		if(Other.CompareTag("B") && four == true)
		{
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logCorrectTime();
			correctNotify2.SetActive(true);
			GameObject.Find("BookDetector1").GetComponent<Detect1>().logChange();
	...
	IEnumerator WaitForAnotherSec() {
		...
		Change.text = "";
		r1.text = "Ascending";
		r2.text = "Uppercase Then Lowercase Letter";
		yield return new WaitForSeconds(2);
		d1.text = "F";
		d2.text = "e";
		d3.text = "b";
